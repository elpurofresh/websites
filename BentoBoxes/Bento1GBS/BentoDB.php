	...
	while($row = mysql_fetch_array($result)) {
		...
		if(($count_row[NoOfRecordings]-$count)<=24){
			...
			for($column=3; $column<sizeof($row); $column++){
				//$deltaTemp = abs($row[$column]-$prev[$column]);
				//echo "previous: " . $prev[$column] . " current: " .$row[$column] ." deltaVal = " .$deltaTemp."<br />";
				//$maxDeltaTemp = 30;
				//if(($deltaTemp >= $maxDeltaTemp) || ($row[$column]<=0) || ($row[$column]>=100) ){	
