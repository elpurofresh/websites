<?php  
 ...
 include("./pChart.class");
	...
	#return: 
	#
	# Function to export all the table contents to recordings2.csv file and the last 24 rows to recordings242.csv
	#	
	###############################################################################################################################
    function function_exportToCSV($hostname, $db_user, $db_password, $db_table, $database) {
        echo "in function_exportToCSV!!";
         
        //open files to write
	$file = "recordings2.csv";
	$file2 = "recordings242.csv";
	...
	$result = mysql_query("SELECT * FROM `BENTOBOX2` ORDER BY  `BENTOBOX2`.`RecordTime` ASC") or die(mysql_error());
	$count_result = mysql_query("SELECT count(*) AS 'NoOfRecordings' FROM $db_table") or die(mysql_error());
        $count_row = mysql_fetch_array($count_result);	
	$count = 0;
	$prev=array("2012-04-19 17:35",10.5,"Normal",77.4,47.38,82.61,30.46,82.4,70.28,81.28,72.42,72.52,73.76,40.27,45.94,66.54,34.45);
	//fetch the data row-wise and insert , between the fields and write to the file
	while($row = mysql_fetch_array($result)) {
		$string_row = $row[RecordTime].",".$row[BatteryPercent].",".$row[Rate].",".$row[Temperature1].",".$row[Temperature2].",".$row[Temperature3].",".$row[Temperature4].",".$row[Temperature5].",".$row[Temperature6].",".$row[Temperature7].",".$row[Temperature8].",".$row[Temperature9].",".$row[Temperature10].",".$row[Temperature11].",".$row[Temperature12].",".$row[Temperature13].",".$row[Temperature14]."\n";
	...
	function function_displayGraph(){
		// Dataset definition      
		$DataSet = new pData;     
		$DataSet->ImportFromCSV("./recordings242.csv",",",array(3,4,5,6,7,9,10,11,12,13,14,15),FALSE,0);     
   ...
   $size=sizeof($overview);
    echo "<title>Bentobox2</title>
        <META HTTP-EQUIV=\"REFRESH\" CONTENT=\"300\">
        <h1>BentoBox2</h1><br><br> Select New Rate :
	...
	echo "<tr bgColor = \"#F0F0F0\"><th colspan=\"2\">TimeStamp</th><th colspan=\"2\">Battery (V)</th><th colspan=\"2\">Rate</th><th>Temperature 1</th><th>Temperature 2</th><th>Temperature 3</th><th>Temperature 4</th><th>Temperature 5</th><th>Temperature 6</th><th>Temperature 7</th><th>Temperature 8</th><th>Temperature 9</th><th>Temperature 10</th><th>Temperature 11</th><th>Temperature 12</th><th>Temperature 13</th><th>Temperature 14</th></tr>";	
   ...
   for($i=0;$i<$size;$i++){
		...
		$subj=$val->subject;
		$seen=$val->seen;
		if($subj != "SBD Msg From Unit: 300234011909540"){
			continue;
		}
        if($seen == 1){
            continue;
	   ...
	   // MAKE DANISH DATE DISPLAY
		...
		while(sizeof($arrtemp)!=14){
            array_push($arrtemp,100);
		}
		
            echo "msgno: ".$msg." Seen: ".$seen."<br>";
			$sql = "INSERT INTO $db_table(RecordTime,BatteryPercent,Rate,Temperature1,Temperature2,Temperature3,Temperature4,Temperature5,Temperature6,Temperature7,Temperature8,Temperature9,Temperature10,Temperature11,Temperature12,Temperature13,Temperature14) values ('".$timestamp."','".$batteryPercent."','".$rate."','".$arrtemp[0]."','".$arrtemp[1]."','".$arrtemp[2]."','".$arrtemp[3]."','".$arrtemp[4]."','".$arrtemp[5]."','".$arrtemp[6]."','".$arrtemp[7]."','".$arrtemp[8]."','".$arrtemp[9]."','".$arrtemp[10]."','".$arrtemp[11]."','".$arrtemp[12]."','".$arrtemp[13]."')";
