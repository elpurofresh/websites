...
function function_exportToCSV($hostname, $db_user, $db_password, $db_table, $database) {
	...
	while($row = mysql_fetch_array($result)) {
		$string_row = $row[RecordTime].",".$row[BatteryPercent].",".$row[Rate].",".$row[WindSpeedAverage].",".$row[WindDirectionAverage].",".$row[AirPressure].",".$row[AirTemperature].",".$row[RelativeHumidity].",".$row[RainAccumulation].",".$row[CO2ppm].",".$row[SO2ppm]."\n";
...
function function_displayWindSpeedGraph(){
	...
	$DataSet->SetSerieName("Wind Speed","Serie3");
...
for($i=0;$i<$size;$i++){
	...
	$sql = "INSERT INTO $db_table(`RecordTime`, `BatteryPercent`, `Rate`, `WindSpeedAverage`, `WindDirectionAverage`, `AirPressure`, `AirTemperature`, `RelativeHumidity`, `RainAccumulation`, `CO2ppm`, `SO2ppm`) values ('".$timestamp."','".$batteryPercent."','".$rate."','".$arrtemp[0]."','".$arrtemp[1]."','".$arrtemp[2]."','".$arrtemp[3]."','".$arrtemp[4]."','".$arrtemp[5]."','".$arrtemp[8]."','".$arrtemp[9]."')";
